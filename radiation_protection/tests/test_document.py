...
def mock_document_fixture():
    ...
    doc.add_paragraph("Start: ${first_run_date_start}")
    doc.add_paragraph("End: ${first_run_date_end}")
    return doc
...
def test_prepare_variables_with_run(quiz_result: QuizResult, next_user_run: Run):
    ...
    assert variables["${first_run_date_start}"] == next_user_run.start_date.strftime(  # type: ignore # pylint: disable=line-too-long
        "%d/%m/%Y"
    )

    assert variables["${first_run_date_end}"] == next_user_run.end_date.strftime(  # type: ignore # pylint: disable=line-too-long
        "%d/%m/%Y"
    )
...
def test_prepare_variables_without_run(quiz_result: QuizResult):
    ...
    assert variables["${first_run_date_start}"] == ""
    assert variables["${first_run_date_end}"] == ""
...
def test_replace_variables_in_document(mock_document: "DocumentObject"):
    ...
    variables = {
        ...
        "${first_run_date_start}": "01/01/2024",
        "${first_run_date_end}": "02/01/2024",
    }
