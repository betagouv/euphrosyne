document.addEventListener("DOMContentLoaded", function () {
  ...
  function dismissAddRelatedRunPopup(win, newId, newRepr, data) {
    ...
    newInline.querySelector(".inline_label").appendChild(
      new DOMParser().parseFromString(
        `
      <a href="/admin/lab/run/${newId}/change/?_popup=1" class="inlinechangelink related-run-link">Modification</a>
      `,
        "text/html"
  ...
  function dismissChangeRelatedRunPopup(win, objId, newRepr, newId, newData) {
    // We consider that the newId will never change.
    const inline = Array.from(
      document.querySelectorAll("#runs-group .inline-related")
    ).filter(
      (n) =>
        n.querySelector("a.inlinechangelink") &&
        new RegExp(`.*/lab/run/${objId}/.*`).test(
          n.querySelector("a.inlinechangelink").href
        )
    )[0];
    inline.querySelector(".inline_label").childNodes[0].data = newData.label;
    [
      "start_date",
      "end_date",
      "particle_type",
  ...
  function dismissDeleteRelatedRunPopup(win, objId) {
    const inline = Array.from(
      document.querySelectorAll("#runs-group .inline-related")
    ).filter(
      (n) =>
