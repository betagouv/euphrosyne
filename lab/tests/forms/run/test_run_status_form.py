...
def test_creation_requires_no_fields():
    form = forms.RunStatusBaseForm(data={"status": models.Run.Status.CREATED})
    assert form.is_valid()


@pytest.mark.parametrize(
    "status",
    [s for s in models.Run.Status if s != models.Run.Status.CREATED],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        if s not in [models.Run.Status.ASK_FOR_EXECUTION, models.Run.Status.CREATED]
    ],
)
